...
def proyectos_fastapi() -> rx.Component:
    return rx.box(
        rx.vstack(
            rx.box(
                rx.heading("PROYECTOS FASTAPI", class_name=PROYECTOS_HEADER_TITULO_PRINCIPAL),
                class_name=PROYECTOS_HEADER_BASE,
            ),
            rx.box(
                card_proyecto(INFORMACION_PROYECTOS["proyecto_fastapi_1"]),
                card_proyecto(INFORMACION_PROYECTOS["proyecto_fastapi_2"]),
